...
export default function Home() {
    return (
        <main className="min-h-screen bg-header bg-no-repeat bg-cover bg-center bg-fixed my-0 mx-0 text-center h-max w-full flex flex-col text-white">
            <div className="text-3xl w-full px-14 flex flex-row text-white font-bold items-end z-10">
                PanRadio <span className="text-5xl px-4 text-space-purple">GRB</span> Project Description
            </div>


            <div className="flex flex-col pb-9 flex-1 text-justify mx-32 text-xl bg-black/75 px-10 mt-6 mb-6 rounded-xl z-10">
                {/* FIRST SECTION */}
                <h1 className="text-left mx-10 mb-12 mt-6 w-fit text-3xl text-white font-bold text-shadow-sm shadow-white">GRBs and their radio afterglows</h1>
                ...
                {/* SECOND SECTION */}
                <h1 className="text-left mx-10 my-12 w-fit text-3xl text-white font-bold text-shadow-sm shadow-white">Science Goals</h1>
                <p>Over 4 years we will use ATCA to establish a complete and unbiased, multi-frequency and high-cadence radio view of GRBs, which will be used to address the following fundamental questions: </p>
                <ol type='1' className='list-decimal px-12 text-lg py-2'>
                    <li>
                ...
                <p>In particular, early- and late-time radio, on which these questions rely are very seldom explored. Our program will help to answer these questions by doubling the number of GRBs with comprehensive radio monitoring, from early (within a day) through to late (years) time. </p>

                {/* THIRD SECTION */}
                <h1 className="text-left mx-10 my-12 w-fit text-3xl text-white font-bold text-shadow-sm shadow-white">Observing Strategy</h1>
                <p>Beginning in the 2023 April ATCA observing semester, we expect to observe 10 long and 2 short GRBs per year over 4 years below a declination of -10 deg, obtaining an unbiased radio sample from early (less than 24 hours) to late (months to years) times post-burst. We are using the rapid-response observing system on ATCA to automatically and rapidly trigger observations of GRBs within minutes of their discovery (or when they have risen above the horizon), collecting the very earliest light emitted by these events. We then continue to monitor their radio afterglows from days to weeks post-burst over multiple radio frequencies using ATCA (1-50 GHz). We will continue to monitor any radio afterglow with ATCA until they fade below detectability. </p>
                
                
                {/* FOURTH SECTION */}
                <h1 className="text-left mx-10 my-12 w-fit text-3xl text-white font-bold text-shadow-sm shadow-white">Australia Rapid-response radio telescopes</h1>
                <p>Both the Australia Telescope Compact Array (ATCA) and the Murchison Widefield Array (MWA) are equipped with rapid-response observing systems, which allow them to automatically respond to transient alerts broadcast via the General Coordinates Network causing them to repoint and begin observing the event between seconds to minutes post-burst. </p>
